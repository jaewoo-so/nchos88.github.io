...
메틸레이션만 사용한 데이터, 즉 RNA데이터를 사용하지 않았을 경우 STES를 HNSC로 잘못 예측하는 경우가 늘어납니다. 반대로 RNA만 사용한 데이터, 즉 메틸레이션 데이터를 사용하지 않았을 경우 UCS를 SARC로 잘못 예측하는 경우가 늘어납니다. 따라서 암종 중에 메틸레이션이나 RNA중 한가지에 치우친 고유한 패턴을 가진 암종이 있을 거란 가능성이 높다고 유추해 볼 수 있었습니다. (neural network 및 xgboost에서도 같은 결과)
